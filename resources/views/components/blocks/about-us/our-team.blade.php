...
<div {{ $attributes }}>
    <x-typography.guest-page-header size="h2" class="mb-12" title="{{ __('Our Team') }}"
        description="{{ __('Our talented team that actually does not exist. But we pretend it does to make it look like we have a team.') }}" />

    <div class="mx-auto mt-8 flex max-w-5xl flex-wrap items-center justify-center gap-16 p-8 lg:justify-between">
        @foreach($teamMembers as $member)
            <div class="flex flex-col items-center gap-2">
                <div class="relative size-44 overflow-hidden rounded-full md:size-52">
                    <img 
                        src="{{ $member['image'] }}"
                        alt="{{ $member['name'] }}"
                        class="size-44 origin-top rounded-full grayscale transition-all duration-500 hover:scale-110 hover:grayscale-0 md:size-52"
                        loading="lazy"
                        decoding="async"
                    >
                </div>
