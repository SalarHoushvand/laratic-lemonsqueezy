...
<div
    x-data="{
        ...
        setSelectedOption(option) {
            ...
            // Set the country code in the input and trigger Livewire sync
            const input = this.$refs.phoneNumberInput;
            input.value = option.phoneCode + ' ';
    ...
    x-init="
        options = allOptions;
        selectedOption = allOptions.find(option => option.iso === '{{ $defaultCountry }}') || allOptions[0];
    ">
    @if ($label)
        <label
            for="{{ $id }}"
    ...
    <div class="relative flex w-full">
        ...
        <div>
            ...
            <button
                type="button"
                class="inline-flex h-full w-max items-center justify-between gap-2 whitespace-nowrap rounded-l-radius border border-outline border-r-0 bg-surface-alt px-4 py-2 text-sm font-medium tracking-wide text-on-surface transition hover:opacity-75 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-primary disabled:cursor-not-allowed disabled:opacity-75 dark:border-outline-dark dark:bg-surface-dark-alt/50 dark:text-on-surface-dark dark:focus-visible:outline-primary-dark"
                role="combobox"
                ...
                <!-- Chevron -->
                <x-icons.chevron-down size="size-5" strokeWidth="1.5" class="shrink-0" />
            </button>
            ...
            <div
                ...
                id="countriesList"
                class="absolute left-0 top-11 z-10 w-80 overflow-hidden rounded-radius border border-outline bg-surface-alt dark:border-outline-dark dark:bg-surface-dark-alt"
                role="listbox"
                    ...
                    <template x-for="(item, index) in options" x-bind:key="item.iso">
                        <li
                            ...
                            <div class="flex items-center gap-2">
                                ...
                                <span class="sr-only" x-text="selectedOption == item ? '{{ __('selected') }}' : null"></span>
                            </div>
        ...
        <input
            ...
            type="tel"
            name="{{ $name }}"
            class="w-full rounded-r-radius rounded-l-none border border-outline bg-surface-alt px-2.5 py-2 text-sm text-on-surface focus:outline-hidden focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-primary disabled:cursor-not-allowed disabled:opacity-75 dark:border-outline-dark dark:bg-surface-dark-alt/50 dark:text-on-surface-dark dark:focus-visible:outline-primary-dark"
