...
<div x-data="{
    ...
    setSelectedOption(option) {
        ...
        $wire.country = this.selectedOption.value
    },
...
}" {{ $attributes->merge(['class' => 'flex w-full ' . $width . ' flex-col']) }}
    x-on:keydown="handleKeydownOnOptions($event)" x-on:keydown.esc.window="isOpen = false, openedWithKeyboard = false"
    x-init="
        options = allOptions;
        if ($wire.country) {
            selectedOption = allOptions.find(option => option.value === $wire.country)
        }
    ">
    <div class="relative">
        <!-- trigger button  -->
        <button type="button"
            class="{{ $error ? 'border-danger' : 'border-outline dark:border-outline-dark' }} inline-flex w-full items-center justify-between border rounded-radius bg-surface-alt px-2 py-2 text-sm font-medium tracking-wide text-on-surface transition hover:opacity-75 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-primary dark:bg-surface-dark-alt dark:text-on-surface-dark dark:focus-visible:outline-primary-dark"
            ...
            x-bind:aria-label="selectedOption ? selectedOption.value : '{{ __('Please Select') }}'">
            <span class="text-sm font-normal text-ellipsis overflow-hidden w-fit whitespace-nowrap pr-6"
                x-text="selectedOption ? selectedOption.value : '{{ __('Please Select') }}'"></span>
            <!-- Chevron  -->
        ...
        <div x-cloak x-show="isOpen || openedWithKeyboard" id="countriesList"
            ...
            role="listbox" aria-label="countries list" x-on:click.outside="isOpen = false, openedWithKeyboard = false"
                ...
                <template x-for="(item, index) in options" x-bind:key="item.value">
                    <li class="combobox-option inline-flex justify-between gap-6 bg-surface-alt px-4 py-2 text-sm text-on-surface hover:bg-surface-dark-alt/5 hover:text-on-surface-strong focus-visible:bg-surface-dark-alt/5 focus-visible:text-on-surface-strong focus-visible:outline-hidden dark:bg-surface-dark-alt dark:text-on-surface-dark dark:hover:bg-surface-alt/5 dark:hover:text-on-surface-dark-strong dark:focus-visible:bg-surface-alt/10 dark:focus-visible:text-on-surface-dark-strong"
                        ...
                        <div class="flex items-center gap-2">
                            <!-- Flag image  -->
                            <img class="w-5 h-3.5" x-bind:src="'https://flagcdn.com/' + item.iso + '.svg'"
                                alt="" aria-hidden="true" />
                            <!-- Country name  -->
                            <span x-bind:class="selectedOption == item ? 'font-bold' : null" x-text="item.label"></span>
                            <!-- Screen reader 'selected' indicator  -->
                            <span class="sr-only"
                                x-text="selectedOption == item ? '{{ __('selected') }}' : null"></span>
                        </div>
                        <!-- Checkmark  -->

                        <x-icons.check size="md" x-cloak x-show="selectedOption == item" strokeWidth="2" />
                    </li>
                </template>
            </ul>
        </div>
